...
class TF(object):
    ...
    def show_tf_in_file(self, file):
        ...
        f.write("FININSH FIRST WRITE\n")
        for b in self.lazy_eval_bytes:
            f.write("lazy_eval_bytes:\n")
            f.write("%d\n:q"%b)
        f.write("Finish write lazy bytes.\n")
        count = 0
        for rule in self.rules:
            count = count + 1
            f.write("\nNew rule:%d\n"%count)
            f.write("Action of rule:\n")
            f.write("%s$\n"%rule["action"])
            f.write("In ports of the rule:\n")
            f.write("%s$\n"%rule["in_ports"])
            f.write("match:\n")
            ...
            f.write("Inverse match:\n")
            f.write("%s\n"%wildcard_to_str(rule["inverse_match"]))
            f.write("Inverse rewrite:\n")
